		...
		if self.normalized==True:
			...
			if self.first_query == True:
				...
				for i in doc.keys():
					...
					self.sumDocWeights[i] = np.square(np.array([i*i for i in self.Weighter.getWeightsForDoc(i).values()]).sum())
			...
			for stem in query.keys():
				...
				for IdDoc in DicWeigthsDoc.keys():
					if IdDoc in result.keys():
						result[int(IdDoc)] += DicWeigthsDoc[IdDoc]*QueryWeights[stem]/(self.sumDocWeights[IdDoc]*sumQueryWeights)
					else:
						result[int(IdDoc)] = DicWeigthsDoc[IdDoc]*QueryWeights[stem]/(self.sumDocWeights[IdDoc]*sumQueryWeights)
		return result
