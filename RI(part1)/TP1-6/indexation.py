...
class Index():
	...
	def indexation(self, filename):
		...
		while (doc != None):
			...
			#add links
			links =" " + doc.others["links"]
			links_byte_size = len(links)
			stem_byte_size = len(str_bow)
			self.index[id] = (doc_place, stem_byte_size,links_byte_size)
			doc_place += stem_byte_size + links_byte_size
		...
		while (doc != None):
			...
			index_file.write(" " + doc.others["links"])
			#Index inverse
	...
	def getLinksForDoc(self,id_doc):
		index_file = open(self.index_file, 'r')
		index = pkl.Unpickler(open(self.index_file_position,'rb')).load()
		index_file.seek(index[id_doc][0]+index[1])
		links = index_file.read(index[id_doc][2])
		index_file.flush()
		index_file.close()
		return links
